...
pub fn select(db: &sled::Db, hashkey: &str) -> Result<Option<PackEntry>, Error> {
    let value = db.get(hashkey).map_err(|err| Error::SledSelectError { source: err })?;
