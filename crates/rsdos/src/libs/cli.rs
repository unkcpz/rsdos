...
pub fn add_file(
    ...
    let (bytes_streamd, hash_hex) = match to {
        ...
        StoreType::Packs => packs_insert(file.clone(), cnt, &db)?,
    };
...
pub fn stat(cnt: &Container, db: &sled::Db) -> anyhow::Result<ContainerInfo> {
    cnt.valid()?;
...
mod tests {
    ...
    fn cli_add_ten_diff_objs_to_loose() {
        ...
        for i in 0..10 {
            ...
            add_file(&fp.to_path_buf(), &cnt, &db, &StoreType::Loose).expect("unable to add file {i}");
        }

        // status audit
        let info = stat(&cnt, &db).expect("fail to audit container stat");
    ...
    fn cli_add_ten_same_objs_to_loose() {
        ...
        for _i in 0..10 {
            ...
            let fp = tf.into_temp_path();
            let _ = add_file(&fp.to_path_buf(), &cnt, &db, &StoreType::Loose)
    ...
    fn cli_add_ten_diff_objs_to_packs() -> anyhow::Result<()> {
        ...
        let orig_objs: HashMap<String, String> = (0..10)
            .map(|i| {
                ...
                let fp = tf.into_temp_path();
                let (hash_hex, _, _) = add_file(&fp.to_path_buf(), &cnt, &db, &StoreType::Packs)
        ...
        for (hash_hex, expected_content) in orig_objs {
            ...
            let obj = io_packs::extract(&hash_hex, &cnt, &db)?.unwrap();
            assert_eq!(
